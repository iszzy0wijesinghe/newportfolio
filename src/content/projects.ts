...
export type Project = {
  ...
  id: number; // ✅ use numeric id (same as projectDetailsById key)
  title: string;
  short: string;
  description: string;
  tech: string[];
...
export function searchProjects(query: string): Project[] {
  ...
  if (/^\d+$/.test(q)) {
    const id = Number(q);
    const hit = projects.find((x) => x.id === id);
    return hit ? [hit] : [];
  }

  return projects.filter((p) => {
    const details = projectDetailsById[p.id];
